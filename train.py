...
from pscse_model.models import RobertaForCL, BertForCL
from pscse_model.trainers import CLTrainer
...
class ModelArguments:
    ...
    num_neg:int = field(
        default=0,
        metadata={
            "help":"提示负样本扩充num_neg倍"
        }
    )
    only_prompt_neg:bool=field(
        default=False
    )
    sample_filter:bool = field(
        default = False
    )
    ...
    beta:Optional[float]=field(
        default = 0,
        metadata={"help":
                       "Threshold for filtering false-negative."}
    )

    # instance_weight:float = field(
    #     default=0,
    #     metadata={
    #         "help":"The **logit** of weight for negatives according to sim of negatives"
    #     }
    # )
    hard_negative_weight: float = field(
        # 设为正数，增大与hard_neg的相似性
        # 设为负数，减小
        default=0,
        metadata={
    ...
    num_prompt_neg_per_batch: int = field(
        default=1,
        metadata = {
        }
    )
    neg_prompt_sentence_template: Optional[str] = field(
        # default="*cls*_This_sentence_:_\"*sent_0*\"_does_not_mean_*mask*.*sep+*",
        default=None,
        metadata={
        }
    )
    hard_neg_prompt_sentence_template:Optional[str] = field(
        # default="*cls*_This_sentence_:_\"_*sent_0*_\"_maybe_means_*mask*.*sep+*",
        default=None,
    )
    ...
    neg_mask_embedding_sentence_bs:str = field(
        default=' This sentence : "',
        metadata={
        }
    )
    neg_mask_embedding_sentence_es: str = field(
        default='" does not mean [MASK].',
        metadata={
        }
    )
...
def main():
    ...
    if extension == "csv":
        ...
        # datasets = load_dataset(extension, data_files=data_files, cache_dir="./data/")[:32]
    ...
    if model_args.model_name_or_path:
        if 'roberta' in model_args.model_name_or_path:
            ...
            if model_args.sample_filter:
                fix_bert = BertModel.from_pretrained(model_args.c_model_name_or_path)
                model.fix_bert.load_state_dict(fix_bert.state_dict())
    ...
    if model_args.neg_prompt_sentence_template:
        if model_args.neg_prompt_sentence_template != '':
            neg_template = model_args.neg_prompt_sentence_template
            assert ' ' not in neg_template
            neg_template = neg_template.replace('*mask*', tokenizer.mask_token)\
                               .replace('*sep+*', '')\
                               .replace('*cls*', '').replace('*sent_0*', ' ')
            neg_template = neg_template.split(' ')
            model_args.neg_mask_embedding_sentence_bs = neg_template[0].replace('_', ' ')
            if 'roberta' in model_args.model_name_or_path:
                # remove empty block
                model_args.neg_mask_embedding_sentence_bs = model_args.neg_mask_embedding_sentence_bs.strip()
            model_args.neg_mask_embedding_sentence_es = neg_template[1].replace('_', ' ')
    if model_args.hard_neg_prompt_sentence_template:
        if model_args.hard_neg_prompt_sentence_template != '':
            hard_neg_template = model_args.hard_neg_prompt_sentence_template
            assert ' ' not in neg_template
            hard_neg_template = hard_neg_template.replace('*mask*', tokenizer.mask_token)\
                               .replace('*sep+*', '')\
                               .replace('*cls*', '').replace('*sent_0*', ' ')
            hard_neg_template = hard_neg_template.split(' ')
            model_args.hard_neg_mask_embedding_sentence_bs = hard_neg_template[0].replace('_', ' ')
            if 'roberta' in model_args.model_name_or_path:
                # remove empty block
                model_args.hard_neg_mask_embedding_sentence_bs = model_args.hard_neg_mask_embedding_sentence_bs.strip()
            model_args.hard_neg_mask_embedding_sentence_es = hard_neg_template[1].replace('_', ' ')
    ...
    def prepare_features(examples):
        ...
        if model_args.mask_embedding_sentence:
            ...
            for i, s in enumerate(sentences):
                if i < total:
                    ...
                    s = tokenizer.encode(s, add_special_tokens=False)[:data_args.max_seq_length]
                    sent_features['input_ids'].append(bs2+s+es2)
            if not model_args.neg_prompt_sentence_template:
        ...
        if model_args.neg_prompt_sentence_template:
            # 只取sentences中的一半
            sentences_half = sentences[:total]

            bs_neg = tokenizer.encode(model_args.neg_mask_embedding_sentence_bs)[:-1]
            es_neg = tokenizer.encode(model_args.neg_mask_embedding_sentence_es)[1:]  # remove cls or bos
            ...
            neg_sent_features = {'input_ids': [], 'attention_mask': []}
            for i, s in enumerate(sentences_half):
                if i < total:
                    s = tokenizer.encode(s, add_special_tokens=False)[:data_args.max_seq_length]
                    neg_sent_features['input_ids'].append(bs_neg + s + es_neg)
                # elif i < 2 * total:
                #     s = tokenizer.encode(s, add_special_tokens=False)[:data_args.max_seq_length]
                #     sent_features['input_ids'].append(bs2 + s + es2)
                else:
            ...
            ml_neg = max([len(i) for i in neg_sent_features['input_ids']])
            ml_pos = max([len(i) for i in sent_features['input_ids']])
            ml = max(ml_pos,ml_neg)

            pos_len = len(sent_features['input_ids'])
            for i in range(pos_len):
                t = sent_features['input_ids'][i]
                sent_features['input_ids'][i] = t + [tokenizer.pad_token_id] * (ml - len(t))
                sent_features['attention_mask'].append(len(t) * [1] + (ml - len(t)) * [0])
                if i < pos_len / 2:
                    t = neg_sent_features['input_ids'][i]
                    neg_sent_features['input_ids'][i] = t + [tokenizer.pad_token_id] * (ml - len(t))
                    neg_sent_features['attention_mask'].append(len(t) * [1] + (ml - len(t)) * [0])
        # else:
        #     neg_sent_features = tokenizer(
        #         sentences_half,
        #         max_length=data_args.max_seq_length,
        #         truncation=True,
        #         padding="max_length" if data_args.pad_to_max_length else False,
        ...
        elif model_args.neg_prompt_sentence_template:
            half_total = total // 2
            #assert total==2*half_total,"数据量必须是偶数"
            for key in sent_features:
                features[key] = [[sent_features[key][i], sent_features[key][i + total],
                                  neg_sent_features[key][i]]
                                 for i in range(total)]
                # features[key] = [[sent_features[key][i], sent_features[key][i + 2*half_total], neg_sent_features[key][i+half_total]]
    ...
    if model_args.mask_embedding_sentence:
        ...
        model.bs_neg = tokenizer.encode(model_args.neg_mask_embedding_sentence_bs, add_special_tokens=False)
        model.es_neg = tokenizer.encode(model_args.neg_mask_embedding_sentence_es, add_special_tokens=False)
        ...
        model.neg_mask_embedding_template = tokenizer.encode(model_args.neg_mask_embedding_sentence_bs + model_args.neg_mask_embedding_sentence_es)

        print('template bs', model.bs)
        print('template es', model.es)
        print('neg_template bs', model.bs_neg)
        print('neg_template es', model.es_neg)
        ...
        print('template neg_mask_embedding_template', tokenizer.decode(model.neg_mask_embedding_template))
        print('template neg_mask_embedding_template', model.neg_mask_embedding_template)
