"""
Model definitions for PSCSE.
This file corresponds to the final model used in all main experiments.
"""
...
import transformers
from transformers import RobertaTokenizer, AutoModel, RobertaConfig
...
def cl_forward(cls,
    ...
    def get_delta(encoder1, template_token, length=50):
        with torch.set_grad_enabled(not cls.model_args.mask_embedding_sentence_delta_freeze):
            ...
            if cls.model_args.mask_embedding_sentence_autoprompt:
                d_inputs_embeds = encoder1.embeddings.word_embeddings(d_input_ids)
            ...
            outputs = encoder1(input_ids=d_input_ids if d_inputs_embeds is None else None ,
    ...
    if cls.model_args.mask_embedding_sentence_delta:
        delta, template_len = get_delta(encoder,[cls.mask_embedding_template], )
        if encoder_fixed:
            fix_delta, fix_template_len = get_delta(encoder_fixed,[cls.mask_embedding_template])

        if len(cls.model_args.mask_embedding_sentence_different_template) > 0:
            delta1, template_len1 = get_delta(encoder, [cls.mask_embedding_template2])
            if encoder_fixed:
                fix_delta1, fix_template_len1 = get_delta(encoder_fixed,[cls.mask_embedding_template2])
        if cls.model_args.neg_prompt_sentence_template :
            delta_neg, template_len_neg = get_delta(encoder,cls.neg_mask_embedding_template)
            if encoder_fixed:
                fix_delta_neg, fix_template_len_neg = get_delta(encoder_fixed,[cls.neg_mask_embedding_template])
    ...
    if cls.model_args.num_neg >= 1:
        # 提示负样本扩num倍
        #num = cls.model_args.num_neg
        input_ids_pn = input_ids[:, 2, :].unsqueeze(1)
        attention_mask_pn = attention_mask[:, 2, :].unsqueeze(1)
        input_ids_pns = input_ids_pn.repeat(1, int(cls.model_args.num_neg), 1)
        attention_mask_pns = attention_mask_pn.repeat(1, int(cls.model_args.num_neg), 1)
        input_ids = torch.cat([input_ids, input_ids_pns], dim=1)
        attention_mask = torch.cat([attention_mask, attention_mask_pns], dim=1)
    ...
    if cls.model_args.mask_embedding_sentence:
        last_hidden = outputs.last_hidden_state
        #print("last_hidden.device:",last_hidden.device)
        ...
        if cls.model_args.mask_embedding_sentence_delta:
            ...
            if len(cls.model_args.mask_embedding_sentence_different_template) > 0:
                ...
                temp_attention_mask = attention_mask.view(batch_size, num_sent, -1)
                token_truth_len = temp_attention_mask.sum(-1)
                blen = token_truth_len - template_len # denoise之后的length
                pooler_output[:, 0, :] -= delta[blen[:, 0]]  # 进行denoise操作
                blen1 = token_truth_len - template_len1
                pooler_output[:, 1, :] -= delta1[blen1[:, 1]]
                if num_sent >= 3:
                    if cls.model_args.neg_prompt_sentence_template:
                        blen_neg = token_truth_len - template_len_neg
                        pooler_output[:, 2:, :] -= delta_neg[blen_neg[:, 2:]]
                    else:
    ...
    # Hard negative
    if num_sent >= 3:
        # z3 = pooler_output[:4, 2]
        z3 = pooler_output[:, 2:]
        z3 = z3.contiguous().view(batch_size * (cls.model_args.num_neg+1),-1)
    ...
    if cls.model_args.sample_filter:
        sim_outputs = encoder_fixed(
            None if cls.model_args.mask_embedding_sentence_autoprompt else input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
        ...
        if cls.model_args.mask_embedding_sentence:
            last_hidden = sim_outputs.last_hidden_state
            
            sim_pooler_output = last_hidden[input_ids == cls.mask_token_id]
            #  这个是进行denoise操作
            if cls.model_args.mask_embedding_sentence_delta:
                if cls.model_args.mask_embedding_sentence_org_mlp:
                    sim_pooler_output = cls.mlp(sim_pooler_output)

                if len(cls.model_args.mask_embedding_sentence_different_template) > 0:
                    sim_pooler_output = sim_pooler_output.view(batch_size, num_sent, -1)
                    attention_mask = attention_mask.view(batch_size, num_sent, -1)
                    token_truth_len = attention_mask.sum(-1)
                    blen = token_truth_len - template_len  # denoise之后的length
                    if cls.model_args.neg_prompt_sentence_template:
                        sim_pooler_output[:, 0, :] -= fix_delta_neg[blen[:, 0]]  # 进行denoise操作
                    blen1 = token_truth_len - template_len1
                    sim_pooler_output[:, 1, :] -= fix_delta1[blen1[:, 1]]
                    # sim_pooler_output[:, 1, :] -= delta1[blen[:, 1]]
                    if num_sent >= 3:
                        if cls.model_args.neg_prompt_sentence_template:
                            blen_neg = token_truth_len - template_len_neg
                            # sim_pooler_output[:, 2, :] -= delta_neg[blen[:, 2]]
                            sim_pooler_output[:, 2:, :] -= fix_delta_neg[blen_neg[:,2:]]
                        else:
                            sim_pooler_output[:, 2, :] -= fix_delta1[blen[:, 2]]
                else:
                    blen = attention_mask.sum(-1) - template_len
                    sim_pooler_output -= fix_delta[blen]

            sim_pooler_output = sim_pooler_output.view(batch_size * num_sent, -1)
        sim_pooler_output = sim_pooler_output.view((batch_size, num_sent, sim_pooler_output.size(-1)))  # (bs, num_sent, hidden)
        if cls.model_args.mask_embedding_sentence_delta and cls.model_args.mask_embedding_sentence_org_mlp:
            # ignore the delta and org
            pass
        else:
            sim_pooler_output = cls.mlp(sim_pooler_output)
            # Separate representation
        z1_sim, z2_sim = sim_pooler_output[:, 0], sim_pooler_output[:, 1]

        # Hard negative
        if num_sent >= 3:
                # z3 = pooler_output[:4, 2]
            z3_sim = sim_pooler_output[:, 2:]
            z3_sim = z3_sim.contiguous().view(batch_size * (cls.model_args.num_neg + 1), -1)

        # Gather all embeddings if using distributed training
        if dist.is_initialized() and cls.training:
            # Gather hard negative
            if num_sent >= 3:
                z3_list = [torch.zeros_like(z3_sim) for _ in range(dist.get_world_size())]
                dist.all_gather(tensor_list=z3_list, tensor=z3_sim.contiguous())
                z3_list[dist.get_rank()] = z3_sim
                z3_sim = torch.cat(z3_list, 0)

            # Dummy vectors for allgather
            z1_list = [torch.zeros_like(z1_sim) for _ in range(dist.get_world_size())]
            z2_list = [torch.zeros_like(z2_sim) for _ in range(dist.get_world_size())]
            # Allgather
            dist.all_gather(tensor_list=z1_list, tensor=z1_sim.contiguous())
            dist.all_gather(tensor_list=z2_list, tensor=z2_sim.contiguous())

            # Since allgather results do not have gradients, we replace the
            # current process's corresponding embeddings with original tensors
            z1_list[dist.get_rank()] = z1_sim
            z2_list[dist.get_rank()] = z2_sim
            # Get full batch embeddings: (bs x N, hidden)
            z1_sim = torch.cat(z1_list, 0)
            z2_sim = torch.cat(z2_list, 0)
        if cls.model_args.dot_sim:
            s_cos_sim = torch.mm(torch.sigmoid(z1_sim), torch.sigmoid(z2_sim.permute(1, 0)))
        else:
            s_cos_sim = cls.sim(z1_sim.unsqueeze(1), z2_sim.unsqueeze(0))
            # Hard negative
            if cls.model_args.norm_instead_temp:
                s_cos_sim *= cls.sim.temp
                cmin, cmax = s_cos_sim.min(), s_cos_sim.max()
                s_cos_sim = (s_cos_sim - cmin) / (cmax - cmin) / cls.sim.temp

            if num_sent >= 3:
                s_z1_z3_cos = cls.sim(z1_sim.unsqueeze(1), z3_sim.unsqueeze(0))
                s_cos_sim = torch.cat([s_cos_sim, s_z1_z3_cos], 1)

        # instance_weight = s_cos_sim*cls.model_args.instance_weight + 10
        # 正例先置为0
        for i in range(len(s_cos_sim)):
            s_cos_sim[i][i] = 0
        # 若相似性大于等于0.9，记作True
        # >= ge     > gt        <= le       <lt
        weight = s_cos_sim.ge(cls.model_args.beta / cls.model_args.temp)
        # for i in range(s_cos_sim.shape[0]):
        #     for j in range(s_cos_sim.shape[1]):
        #         if s_cos_sim[i][j] > 18:
        #             s_cos_sim[i][j] =0


        # 正例不加权重
        # for i in range(s_z1_z3_cos.size(-1)):
        #     instance_weight[i][i] = 0
    ...
    # if cls.model_args.instance_weight:
    #     cos_sim = cos_sim + instance_weight
    ...
    if cls.model_args.sample_filter:
        #for i in range(cos_sim.shape[0]):
            #weight[i][i] = True
        #cos_sim[weight]=0
        #cos_sim[weight].detach().requires_grad_(False)
        cos_sim[weight] = cos_sim[weight].detach().requires_grad_(False)
    if num_sent >= 3 and cls.model_args.hard_negative_weight:
        ...
        # weights = torch.tensor(
        #     [[0.0] * (cos_sim.size(-1) - z1_z3_cos.size(-1)) + [z3_weight] * z1_z3_cos.size(-1)] * z1_z3_cos.size(0)
        # ).to(input_ids.device)
        cos_sim = cos_sim + weights

        # cos_sim = cos_sim.reshape(batch_size,batch_size*(num_sent-1))
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        self.fix_bert = None
        if self.model_args.sample_filter:
    ...
    def forward(self,
        ...
        if sent_emb:
            ...
            return cl_forward(self, self.bert,self.fix_bert,
                input_ids=input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
...
class RobertaForCL(RobertaPreTrainedModel):
    ...
    def forward(self,
        ...
        if sent_emb:
            ...
            return cl_forward(self, self.roberta,self.fix_bert,
                input_ids=input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
